...
BigInt10::BigInt10(const char *str) { initFromString(str, std::strlen(str)); }
...
bool BigInt10::operator!=(const BigInt10 &b) const { return !(*this == b); }
...
void BigInt10::insertZroes(int n) { limbs.insert(limbs.begin(), n, 0); }
...
BigInt10 longDivision(const BigInt10 &a, const BigInt10 &b,
                      BigInt10 &remainder) {
  // c=a/b;
  if (b.isZero())
  ...
  for (int i = a.limbsCount() - 1; i >= 0; i--) { // msb-lsb
    ...
    if (!remainder.isSmallerThanUnsigned(b)) {
      ...
      while (left <= right) {
        ...
        if (!remainder.isSmallerThanUnsigned(b * mid)) { // b*times<=remainder
          times = mid;
          if (left == right)
            break;
          left = mid + 1;
